...
"""
This class uses Long Short Term Memory Neural Network to classify the gender 
"""
import keras
import json
import numpy as np
...
class LSTM(object):
       ...
       def train_lstm(self,X_train,y_train,X_val, y_val,target_class,embedding_matrix, attr2index, term2index, dim_embedding):
            '''
            ...
            rnn = LSTM(100, activation='tanh', use_bias=True, recurrent_initializer='orthogonal')(z)
            z = Dense(50, activation="sigmoid")(rnn)
...
def test_lstm(X_test, model):
          '''
           @Input : Test data and the model
           @Input_type : 2D Numpy array of shape, sequential model
           @Output : predicted probabilty of the classes, predicted highest probabilty
...
def score(probs,preds,y_test, target_class, attr2index):
          '''
           @Input : predicted probabilty of the classes, highest probabilty, Test class data
...
def main():
  ...
  lstm= LSTM(X_train,y_train,X_val, y_val, args.target_class,embedding_matrix, attr2index, term2index, args.dim_embedding)
  model = lstm.train_lstm(X_train,y_train, X_val, y_val, args.target_class, embedding_matrix, attr2index, term2index, args.dim_embedding)
  probs, preds = test_lstm(X_test, model)
