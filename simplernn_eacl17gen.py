...
"""
This class uses Simple Recurrent Neural Network to classify the gender 
"""
import keras
import json
import numpy as np
...
class SimpleRNN(object):
       ...
       def train_RNN(self,X_train,y_train,X_val, y_val,target_class,embedding_matrix, attr2index, term2index, dim_embedding):
            '''
            ...
            rnn = SimpleRNN(128, activation='tanh', use_bias=True, recurrent_initializer='orthogonal')(z)
...
def main():
  ...
  rnn= SimpleRNN(X_train,y_train,X_val, y_val, args.target_class,embedding_matrix, attr2index, term2index, args.dim_embedding)
  model = rnn.train_RNN(X_train,y_train, X_val, y_val, args.target_class, embedding_matrix, attr2index, term2index, args.dim_embedding)
  probs, preds = test_RNN(X_test, model)
